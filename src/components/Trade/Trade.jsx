	...
	return (
		<div>
			<Grid
				...
				h='calc(100vh - 78px)'
			>
				<GridItem bg='#0f131a' area={'to1'} height={'100%'} >
				...
				<GridItem overflow='auto' bg='#0f131a' area={'to2'} >
				...
				<GridItem bg='#343a40' area={'nav'}  >
					{!selectedTrade && <Box>
							...
							{mode !== 'new' && <Flex justify='space-around' >
								...
								<ButtonGroup>
									...
									<Button
										...
										leftIcon={past ? <TeamAvatar size={'xs'} team={team} /> : <FaHandshake style={{ margin: '4px', }} />}
										variant={past ? "solid" : "outline"}
										style={{ backgroundColor: getFadedColor(team.code) }}
									>
						...
						{mode === 'new' && <div className='styleCenter'>
							<SelectPicker label={'name'} placeholder='Select Trade Partner' data={teams.filter(el => el._id !== team._id && el.code !== 'CTL')} onChange={setPartner} />
