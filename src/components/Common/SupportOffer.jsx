...
import { agendaValue, getTextColor } from '../../scripts/frontend';
...
const SupportOffer = (props) => { //trade object
  ...
  return (
    <div className='trade' style={{ width: "100%", padding: '8px', height: 'calc(100vh - 190px)', overflow: 'auto', borderColor: 'inherit', border: '2px solid', textAlign: 'center' }}>
        ...
        {!disabled && resources.map((resource, index) => (
          <InputGroup key={index} className='styleCenter'>
            ...
            <SelectPicker
              ...
              data={props.myAccount.resources.filter(el =>
                el.balance > 0 &&
                !resources.some(r => r.type === el.type) &&
                option.acceptedResources.some(rt => rt.type == el.type))}
            ...
            <h3>+{agendaValue(resource.type, resource.value)}</h3>
      ...
      {!disabled && <IconButton
        onClick={() => setResources([...resources, { value: 0, type: option.acceptedResources.filter(el => !resources.some(r => r.type === el.type))[0]?.type }])}
        variant="solid"
        colorScheme='green'
        size="sm"
        icon={<Plus />}
      ...
      {<SimpleGrid style={{ minHeight: '20vh' }} minChildWidth='200px' spacing='20px' align={'center'}>
        ...
        {!disabled &&
          <AddAsset
            assets={myAssets.filter(el => !assets.some(a => a._id == el._id))}
