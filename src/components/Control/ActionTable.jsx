...
const ActionTable = () => {
    ...
    if (filter) {
      switch (filter) {
        ...
        case 'unresolved':
          ...
          break;
        case 'unassigned':
          filtered = actions.filter((el) => el.control?._id === '' || !el.control);
          setDataToDisplay(filtered);
          break;
        default:
          filtered = actions.filter((el) => el.control?._id === filter);
          setDataToDisplay(filtered);
      }
  ...
  useEffect(() => {
    let sorted = [];
    if (sort) {
      if (sort === 'control') sorted = _.sortBy(dataToDisplay, 'control.characterName');
  ...
  const handleFilter = (filter, controllerId) => {
    if (filter !== 'control') setFilter(filter);
    ...
    switch (control) {
      ...
      default:
        actions
          .filter((el) => el.round === round)
          .forEach((action) => {
            if (action.control?._id === control) count++;
          });
        return count;
    }
  };
  ...
  return (
    <Box>
      ...
      <Box borderWidth='3px' borderRadius='md' borderColor='teal' padding={8}>
        ...
        <Divider />
        <HStack spacing='24px'>
          {controlChars.map((control) => (
            <div key={control._id}
              style={{
                backgroundColor: getFadedColor(control._id == filter ? 'Other' : 'The Overlord'),
                padding: '3px',
                borderRadius: '5px'
              }} >
              <Text as='b' onClick={() => handleFilter('control', control._id)} cursor='pointer'>
                {control.characterName}:
              </Text>
              <Text onClick={() => handleFilter('control', control._id)} cursor='pointer'>
                {getActionCount(control._id)}
              </Text>
            </div>
          ))}
          <div>
      ...
      <Grid templateColumns={templateColumns} gap={4} paddingLeft={8} paddingRight={8} align='left'>
        ...
        <GridItem>
          <Text fontSize='lg' as='b' onClick={() => handleSort('control')} cursor='pointer'>
            Assigned Control
          </Text>
        </GridItem>
        <GridItem>
      ...
      {dataToDisplay
        ...
        .map((item, index) => (
          <Grid style={{ backgroundColor: index % 2 === 1 ? '#0f131a' : '#222936', padding: '8px 0 8px 0' }} justify={'center'} key={item._id} templateColumns={templateColumns} gap={2} paddingLeft={8} paddingRight={8} align='left' >
            ...
            <GridItem>
              <SelectPicker
                value={item.control?.characterName}
                data={controlChars}
                valueKey='_id'
                label='characterName'
